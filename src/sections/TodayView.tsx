...
function TaskRow({ task, onToggle, onRemove }: { task: Task; onToggle: (id: string) => void; onRemove: (id: string) => void }) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl border px-3 py-3 ${meta.soft} ${task.done ? 'opacity-60' : ''}`}>
      <button onClick={() => onToggle(task.id)} className="shrink-0 active:scale-90 transition-transform" aria-label="完成">
      ...
      <button onClick={() => onRemove(task.id)} className="shrink-0 p-1 text-gray-300 active:text-red-400" aria-label="删除">
...
export default function TodayView({ tasks, onToggle, onRemove }: Props) {
    ...
    <div className="px-4 pb-28">
      ...
      {overdue.length > 0 && (
        <>
          ...
          <div className="space-y-2">{overdue.map((t) => <TaskRow key={t.id} task={t} onToggle={onToggle} onRemove={onRemove} />)}</div>
        </>
      )}
      ...
      {active.length > 0 && (
        <>
          ...
          <div className="space-y-2">{active.map((t) => <TaskRow key={t.id} task={t} onToggle={onToggle} onRemove={onRemove} />)}</div>
        </>
      )}
      ...
      {done.length > 0 && (
        <>
          ...
          <div className="space-y-2">{done.map((t) => <TaskRow key={t.id} task={t} onToggle={onToggle} onRemove={onRemove} />)}</div>
        </>
      )}
